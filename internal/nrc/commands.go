...
var Registry = []CommandSpec{
	...
	},
	{
		...
		DataType:    12,
		Direction:   DirectionRequest,
		Description: "添加一个 PROFINET 设备到配置中（仅 RefGSD + DAP_ID；DeviceName/IP 由 C++ 端自动生成，传入无效）",
		Risk:        RiskWrite,
		Response:    nil,
		...
		Args: []ArgumentSpec{{
			Name: "data", Required: true,
			Description: "添加设备的业务字段 JSON（仅 RefGSD + DAP_ID；DeviceName/IPAddress 由 C++ 自动生成，传入会被忽略）",
			Fields: []FieldSpec{
				{Name: "RefGSD", Type: "string", Required: true, Description: "GSDML 文件名", Example: "GSDML-V2.4-HERON-12345678.xml"},
				{Name: "DAP_ID", Type: "string", Required: true, Description: "设备接口 ID（16 进制）", Example: "0x00000010"},
			},
		}},
	...
	},
	{
		...
		DataType:    12,
		Direction:   DirectionRequest,
		Description: "设置配置中一个分散设备（DecentralDevice）的网络参数（所有字段必传；不修改则传 device list 中的原值）",
		Risk:        RiskWrite,
		Response:    nil,
		...
		Args: []ArgumentSpec{{
			Name: "data", Required: true,
			Description: "分散设备的完整网络参数（全部字段必传；不修改的字段传 device list 原值）",
			Fields: []FieldSpec{
				{Name: "SetPNDeviceNum", Type: "int", Required: true,
					Description: "目标设备索引号（1-based，对应 device-list DecentralDevice[] 下标+1）"},
				{Name: "DeviceName", Type: "string", Required: true,
					Description: "设备名称（必须传，不修改则传 device list 中的原值）"},
				{Name: "IPAddress", Type: "string", Required: true,
					Description: "设备 IP 地址（必须传，不修改则传 device list 中的原值）"},
				{Name: "SubnetMask", Type: "string", Required: true,
					Description: "子网掩码（必须传，不修改则传 device list 中的原值）"},
				{Name: "ReductionRatio", Type: "int", Required: true,
					Description: "设备更新周期（必须传，不修改则传 device list 中的原值）"},
				{Name: "SetInTheProject", Type: "bool", Required: true,
					Description: "是否在项目中配置 IP 地址（true 时 IPAddress/SubnetMask 传入生效；false 时IPAddress/SubnetMask 传入不生效）"},
			},
		}},
